...
public class BucketBlockColoredCement extends BucketBlockFull {
    ...
    private void tipBucket(World w, int x, int y, int z, int facing) {
        ...
        if (w.isAirBlock(tx, ty, tz) || !w.getBlockMaterial(tx, ty, tz).isSolid()) {
            ...
            w.setBlockAndMetadataWithNotify(tx, ty, tz, ECBlocks.coloredCement.blockID, color);
            w.scheduleBlockUpdate(tx, ty, tz,
                    ECBlocks.coloredCement.blockID,
                    ((BlockColoredCement) ECBlocks.coloredCement).tickRate(w));
        }
    ...
    public void onBlockClicked(World w, int x, int y, int z, EntityPlayer player) {
        ...
        int meta = w.getBlockMetadata(x, y, z);
        if (!player.capabilities.isCreativeMode && ExtendedCraftItems.colored_cement_bucket != null) {
            int color = getColor(meta);
            ItemStack drop = new ItemStack(ExtendedCraftItems.colored_cement_bucket, 1, color);
    ...
    public void onBlockHarvested(World w, int x, int y, int z, int meta, EntityPlayer player) {
        ...
        if (!w.isRemote && !player.capabilities.isCreativeMode && ExtendedCraftItems.colored_cement_bucket != null) {
            int color = getColor(meta);
            ItemStack drop = new ItemStack(ExtendedCraftItems.colored_cement_bucket, 1, color);
            EntityItem ent = new EntityItem(w, x + 0.5, y + 0.5, z + 0.5, drop);
            w.spawnEntityInWorld(ent);
    ...
    public void registerIcons(IconRegister reg) {
        ...
        for (int i = 0; i < 16; i++) {
            contentIcons[i] = reg.registerIcon("extendedcraft:" + COLOR_NAMES[i] + "_cement");
        }
    }
