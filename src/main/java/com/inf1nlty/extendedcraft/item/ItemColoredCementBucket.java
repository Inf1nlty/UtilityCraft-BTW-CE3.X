...
import com.inf1nlty.extendedcraft.network.CtrlPlacementTracker;
 ...
 */
public class ItemColoredCementBucket extends ECItem {
    ...
    private void placeBucketBlock(World world, EntityPlayer player, ItemStack stack, int color) {
        ...
        ECBlocks.coloredCementBucketBlock.placeColoredBucket(world, x, y, z, color, 0);
    ...
    private void pourCement(World world, EntityPlayer player, ItemStack stack, int color) {
        ...
        if (world.isAirBlock(x, y, z) || !world.getBlockMaterial(x, y, z).isSolid()) {
            ...
            world.setBlockAndMetadataWithNotify(x, y, z, ECBlocks.coloredCement.blockID, color);
            world.scheduleBlockUpdate(x, y, z, ECBlocks.coloredCement.blockID, ECBlocks.coloredCement.tickRate(world));
    ...
    public void registerIcons(IconRegister reg) {
        icons = new Icon[16];
        for (int i=0;i<16;i++) {
            icons[i] = reg.registerIcon("extendedcraft:" + COLOR_NAMES[i] + "_cement_bucket");
        }
        itemIcon = icons[0];
    }
